...
One at a time, inactivity timeout, non-blocking, cancellable.
...
_current_mid  = None
_cancel_event = Event()
_notify_msg   = None   # the message we reply to for status updates
...
def _make_fake_message(magnet: str, title: str):
    """
    Fake message so TaskListener has something to work with.
    Chat is set to owner's DM (PRIVATE).
    up_dest 'pm' tells the bot to send files to the user's PM.
    """
...
class AutoModeTask(TaskListener):
    def __init__(self, magnet: str, title: str):
        ...
        self.is_qbit        = True
        self.is_leech       = True
        self.compress       = True    # -z
        self.is_rss         = True
        self.link           = magnet
        self.name           = title
        self.up_dest        = "pm"    # send to DM - handled natively by bot code
        self.seed           = False
        self.select         = False
        self.extract        = False
        self.join           = False
...
async def _watch_inactivity(mid: int, title: str):
    ...
    await sleep(20)  # give qbt time to register
    ...
    while True:
        ...
        if task is None:
            return False  # finished or already gone
        ...
        try:
            ...
            if speed > 0:
                stalled_for = 0
            elif state in ["metaDL", "checkingResumeData", "checkingDL", "checkingUP"]:
                stalled_for = 0
            elif state in ["uploading", "stalledUP", "forcedUP", "moving"]:
                return False  # uploading = download done
            else:
                stalled_for += check_interval
...
async def _process_queue(trigger_message):
    ...
    await _notify(
        ...
        f"⏱️ {INACTIVITY_TIMEOUT//60}min inactivity timeout\n"
    ...
    while _queue:
        ...
        if not started:
            # task failed to start - skip to next
            await _notify(f"⏭️ Skipping to next item...")
            await sleep(3)
            continue

        # Wait up to 5 seconds for task to appear in task_dict
        registered = False
        for _ in range(10):
            await sleep(0.5)
            async with task_dict_lock:
        ...
        if not registered:
            LOGGER.error(f"[AutoMode] Task never registered in task_dict: {title}")
            await _notify(
                f"❌ <b>Task failed silently:</b>\n<code>{title}</code>\n"
                f"qBittorrent may have rejected the magnet. Skipping..."
            )
            await sleep(3)
            continue
        ...
        # Wait for task to finish or cancel
        while True:
            await sleep(10)
            ...
            if watcher.done():
                ...
                if timed_out:
                    await _notify(
                        f"⏱️ <b>Timed out ({INACTIVITY_TIMEOUT//60}min no progress):</b>\n"
                        f"<code>{title}</code>\nMoving to next..."
                    )
                    await _cancel_mid(_current_mid)
                break
...
async def mode_command(client, message):
    ...
    if cmd_arg == "cancel":
        ...
        await send_message(
            ...
            f"⚙️ One at a time  |  📲 DM  |  ⏱️ {INACTIVITY_TIMEOUT//60}min timeout"
        )
...
async def collect_forwarded(client, message):
    ...
    # Extract best title from message
...
async def start_auto_rss():
    ...
    LOGGER.info(f"[AutoMode] Ready! Timeout={INACTIVITY_TIMEOUT}s | Upload=DM (pm)")
